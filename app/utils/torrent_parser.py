...
class TorrentParser:
    ...
    def parse_magnet_info(self, magnet_url: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {"hash": hash_value, "name": dn or "Unknown", "magnet": magnet_url}
    ...
    def filter_files_by_size(
        ...
        for file in files:
            if file.size < min_size_bytes:
                logger.debug(
                    f"文件 {file.name} 大小 {file.size / 1024 / 1024:.1f}MB 小于最小值 {min_size_mb}MB"
                )
                continue

            if max_size_bytes and file.size > max_size_bytes:
                logger.debug(
                    f"文件 {file.name} 大小 {file.size / 1024 / 1024:.1f}MB 大于最大值 {max_size_mb}MB"
                )
                continue
    ...
    def filter_files_by_type(
        ...
        for file in files:
            # 如果设置了只保留媒体文件(视频+字幕)
            if media_files_only:
                is_media_file = file.is_video or (
                    file.is_subtitle and include_subtitles
                )
    ...
    def filter_smart(
        ...
        """
        filtered_files = []
        has_video_file = any(file.is_video for file in files)
        ...
        for file in files:
            ...
            if skip_subtitle_only and file.is_subtitle and not has_video_file:
    ...
    def apply_filters(
        self, files: List[TorrentFile], filter_settings: Dict
    ) -> List[TorrentFile]:
        """
        应用所有过滤规则
        ...
        if min_size:
            filtered_files = self.filter_files_by_size(
                filtered_files, min_size_mb=min_size, max_size_mb=max_size
            )
