...
def delete_actor_subscription(
    ...
    service.delete_actor_subscription(
        delete_request.subscription_id, delete_request.delete_downloads
    )
    return R.ok()


@router.get(
    "/downloads", response_model=R[List[schema.actor_subscribe.ActorSubscribeDownload]]
)
def get_actor_subscription_downloads(
    actor_id: int, service=Depends(get_actor_subscribe_service)
):
    """获取演员订阅的下载记录"""
...
@router.get(
    "/all-downloads",
    response_model=R[List[schema.actor_subscribe.ActorSubscribeDownloadWithActor]],
)
def get_all_subscription_downloads(service=Depends(get_actor_subscribe_service)):
    """获取所有演员订阅的下载记录"""
...
def run_actor_subscribe():
    """手动执行演员订阅任务"""
    ok = scheduler.manually("actor_subscribe")
    if not ok:
        return R.fail(message="订阅任务不存在，触发失败")
    return R.ok(message="订阅任务已触发，请稍后查看结果")
