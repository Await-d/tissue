...
@router.get(
    "/rules",
    response_model=schema.AutoDownloadListResponse,
    summary="获取自动下载规则列表",
)
async def get_rules(
    ...
    try:
        query = schema.AutoDownloadRuleQuery(
            page=page, page_size=page_size, is_enabled=is_enabled, name=name
        )
...
@router.post(
    "/rules", response_model=schema.AutoDownloadResponse, summary="创建自动下载规则"
)
async def create_rule(
    ...
    try:
        ...
        if (
            hasattr(rule_data, "time_range_type")
            and rule_data.time_range_type
            and isinstance(rule_data.time_range_type, str)
        ):
            rule_data.time_range_type = normalize_time_range_type(
                rule_data.time_range_type
            )
...
@router.put(
    "/rules/{rule_id}",
    response_model=schema.AutoDownloadResponse,
    summary="更新自动下载规则",
)
...
async def delete_rule(
    rule_id: int, service: AutoDownloadService = Depends(get_auto_download_service)
):
    """删除自动下载规则"""
    try:
        service.delete_rule(rule_id)
        return schema.AutoDownloadResponse(
            success=True, message="规则删除成功", data={}
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def get_subscriptions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"日期格式错误: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
...
async def delete_subscription(
    ...
    try:
        ...
        return schema.AutoDownloadResponse(
            success=True, message="订阅记录删除成功", data={}
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
# 统计和管理API
@router.get(
    "/statistics", response_model=schema.AutoDownloadStatistics, summary="获取统计信息"
)
...
@router.post(
    "/trigger", response_model=schema.AutoDownloadResponse, summary="手动触发自动下载"
)
async def trigger_auto_download(
    ...
    try:
        # 生成任务ID
        task_id = f"trigger_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
        ...
        async def background_task():
            ...
            try:
                ...
                with SessionFactory() as db:
                    ...
                    result = task_service.run_cycle(
                        rule_ids=trigger_data.rule_ids, force=trigger_data.force
                    )
                    db.commit()
...
async def get_task_status(task_id: str):
    ...
    return schema.AutoDownloadResponse(
        success=True, message="任务状态查询成功", data=task_info
    )
...
async def test_rule(
    ...
    """测试自动下载规则，预览符合条件的视频"""
    try:
        preview = service.preview_rule(rule_id=rule_id, limit=limit)
        return schema.AutoDownloadResponse(
            success=True, message="规则测试完成", data=preview
        )
    except ValueError as e:
        return schema.AutoDownloadResponse(
            success=False,
...
async def toggle_rule(
    ...
    """启用或禁用自动下载规则"""
    try:
        rule_data = schema.AutoDownloadRuleUpdate(id=rule_id, is_enabled=enabled)
        rule = service.update_rule(rule_data)
        return schema.AutoDownloadResponse(
            success=True,
            message=f"规则已{'启用' if enabled else '禁用'}",
            data={"id": rule.id, "name": rule.name, "is_enabled": rule.is_enabled},
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
