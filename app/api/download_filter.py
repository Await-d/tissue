...
from app.service.download_filter import (
    get_download_filter_service,
...
class FilterSettingsRequest(BaseModel):
    ...
    min_file_size_mb: int = Field(
        default=300, ge=1, le=10000, description="最小文件大小(MB)"
    )
    max_file_size_mb: Optional[int] = Field(
        default=None, ge=1, le=50000, description="最大文件大小(MB)"
    )
    allowed_extensions: Optional[List[str]] = Field(
        default=None, description="允许的文件扩展名列表"
    )
    blocked_extensions: Optional[List[str]] = Field(
        default=None, description="禁止的文件扩展名列表"
    )
    min_seed_count: Optional[int] = Field(default=1, ge=0, description="最小种子数")
    max_total_size_gb: Optional[float] = Field(
        default=None, ge=0, description="种子总大小限制(GB)"
    )
    ...
    media_files_only: bool = Field(
        default=False, description="只保留媒体文件(视频+字幕)"
    )
...
def get_filter_settings(
    ...
    try:
        ...
        settings_dict = {
            ...
            "max_total_size_gb": float(settings.max_total_size_gb)
            if settings.max_total_size_gb
            else None,
...
def save_filter_settings(
    ...
    try:
        ...
        if settings_data["allowed_extensions"]:
            ...
            settings_data["allowed_extensions"] = json.dumps(
                settings_data["allowed_extensions"]
            )

        if settings_data["blocked_extensions"]:
            import json

            settings_data["blocked_extensions"] = json.dumps(
                settings_data["blocked_extensions"]
            )

        # 保存设置
        settings = service.create_or_update_filter_settings(settings_data)

        return R.ok({"id": settings.id, "message": "设置保存成功"})
...
def reset_to_default_settings(
    ...
    try:
        ...
        return R.ok({"id": settings.id, "message": "设置已重置为默认值"})
...
def cleanup_all_torrents(
    ...
    """
    try:
        result = service.cleanup_all_torrents(
            category=category,
            dry_run=dry_run,
