...
class DownloadService(BaseService):
    ...
    def get_downloads(self, include_success=True, include_failed=True):
        logger.info(
            f"开始获取下载列表，参数: include_success={include_success}, include_failed={include_failed}"
        )
        ...
        logger.info(
            f"qBittorrent配置 - 主机: {self.setting.download.host}, 用户名: {self.setting.download.username}, 分类: {self.setting.download.category}"
        )
        ...
        try:
            ...
            if not test_result.get("status", False):
                logger.error(
                    f"qBittorrent连接失败: {test_result.get('message', 'Unknown error')}"
                )
                return []
            ...
            if len(infos) > 0:
                ...
                for i, info in enumerate(infos[:3]):
                    logger.info(
                        f"种子{i + 1}: {info.get('name', 'Unknown')} - 状态: {info.get('state', 'Unknown')} - 标签: {info.get('tags', '')}"
                    )

            category = (
                self.setting.download.category
                if self.setting.download.category
                else None
            )
            ...
            if category:
                ...
                logger.info(
                    f"按分类'{category}'过滤后: {len(infos)} 个种子 (原来 {original_count} 个)"
                )
            ...
            if not include_failed:
                original_count = len(infos)
                infos = [
                    info for info in infos if "整理失败" not in info.get("tags", "")
                ]
                logger.info(
                    f"排除失败种子后: {len(infos)} 个种子 (原来 {original_count} 个)"
                )

            if not include_success:
                original_count = len(infos)
                infos = [
                    info for info in infos if "整理成功" not in info.get("tags", "")
                ]
                logger.info(
                    f"排除成功种子后: {len(infos)} 个种子 (原来 {original_count} 个)"
                )
        ...
        for info in infos:
            torrent = Torrent(
                hash=info["hash"],
                name=info["name"],
                size=utils.convert_size(info["total_size"]),
                path=info["save_path"],
            ...
            filter_result = self.filter_service.filter_torrent_files_readonly(
                info["hash"], files
            )
            if filter_result["success"]:
                ...
                logger.debug(
                    f"种子 {info['hash']} 过滤结果: {len(filtered_files)} 个文件通过过滤"
                )
                ...
                for filtered_file in filtered_files:
                    ...
                    path = (
                        info["content_path"]
                        if len(files) == 1
                        else os.path.join(info["save_path"], filtered_file["path"])
                    )
                    path = self._map_download_path(path)
                    ...
                    try:
                        # 首先检查是否有匹配的种子记录
                        matched_torrent = (
                            self.db.query(DBTorrent)
                            .filter_by(hash=info["hash"])
                            .order_by(DBTorrent.id.desc())
                            .limit(1)
                            .one_or_none()
                        )
                            ...
                            if video_info and video_info.num:
                                ...
                                try:
                                    ...
                                    if video_detail and video_detail.actors:
                                        actors = [
                                            actor.name for actor in video_detail.actors
                                        ]
                                except:
                                    # 如果获取演员信息失败，忽略错误
                                    pass
                ...
                # 如果过滤失败，使用旧的逻辑作为备用
                logger.warning(
                    f"种子 {info['hash']} 过滤失败: {filter_result.get('message', 'Unknown error')}，使用备用逻辑"
                )
                for file in files:
                    # 允许显示任何优先级不为0的文件，无论下载进度如何
                    if file["priority"] == 0:
                    ...
                    path = (
                        info["content_path"]
                        if len(files) == 1
                        else os.path.join(info["save_path"], file["name"])
                    )
                    path = self._map_download_path(path)
        ...
        for i, torrent in enumerate(torrents[:3]):  # 只显示前3个作为示例
            logger.info(
                f"下载任务 {i + 1}: {torrent.name} ({len(torrent.files)} 个文件)"
            )

        return torrents

    def _map_download_path(self, path: str) -> str:
        download_path = (self.setting.download.download_path or "").strip()
        mapping_path = (self.setting.download.mapping_path or "").strip()
        ...
        if mapped_path.startswith("/") and not mapped_path.startswith("//"):
            return mapped_path

        logger.warning(f"映射路径异常，保持原路径: {path} -> {mapped_path}")
        return path

    def complete_download(self, torrent_hash: str, is_success: bool = True):
        self.qb.add_torrent_tags(
            torrent_hash, ["整理成功" if is_success else "整理失败"]
        )
    ...
    def job_scrape_download(cls):
        ...
        with SessionFactory() as db:
            ...
            torrents = download_service.get_downloads(
                include_failed=True, include_success=True
            )
            logger.info(f"获取到{len(torrents)}个下载任务")
            for torrent in torrents:
                if any(tag in ["整理成功", "整理失败"] for tag in torrent.tags):
                    continue
                download_service.scrape_download(
                    video_service, torrent, setting.download.trans_mode
                )
            db.commit()

    def scrape_download(
        self, video_service: VideoService, torrent: Torrent, trans_mode: str
    ):
        has_error = False
    ...
    def job_delete_complete_download(cls):
        with SessionFactory() as db:
            download_service = DownloadService(db=db)
            torrents = download_service.get_downloads(
                include_success=True, include_failed=False
            )
            for torrent in torrents:
                ...
                try:
                    ...
                    for torrent in torrents:
                        ...
                        if checked_count <= 5:
                            ...
                            logger.info(
                                f"  状态: {state}, 进度: {progress:.1%}, 标签: {tags}"
                            )
                            logger.info(
                                f"  检查结果: 完成={is_completed}, 已整理={is_organized}, 做种中={is_seeding}, 丢失文件={is_missing_files}"
                            )
                        ...
                        if is_missing_files:
                            try:
                                # 删除丢失文件的种子（包括数据）
                                download_service.qb.delete_torrent(
                                    torrent["hash"], delete_files=True
                                )
                                deleted_count += 1
                                logger.info(
                                    f"已删除丢失文件种子: {name} (hash: {torrent['hash'][:8]}...)"
                                )
                            except Exception as e:
                        ...
                        should_stop = (
                            is_completed
                            and is_seeding
                            and (is_organized or not has_organize_tag)
                        )
                        ...
                        if should_stop:
                            try:
                                ...
                                stopped_count += 1
                                logger.info(
                                    f"已停止种子做种: {torrent.get('name', 'Unknown')} (hash: {torrent['hash'][:8]}...)"
                                )
                            except Exception as e:
                                logger.error(
                                    f"停止种子做种失败: {torrent.get('name', 'Unknown')} - {str(e)}"
                                )
                    ...
                    if deleted_count > 0:
                        result_messages.append(
                            f"删除了 {deleted_count} 个丢失文件的种子"
                        )

                    if result_messages:
                        logger.info(
                            f"定时检查完成：检查了 {checked_count} 个种子，"
                            + "，".join(result_messages)
                        )
                    else:
                        logger.info(
                            f"定时检查完成：检查了 {checked_count} 个种子，无需处理的种子"
                        )
