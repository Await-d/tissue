...
class SubscribeService(BaseService):
    ...
    def get_subscribes(self):
        return (
            self.db.query(Subscribe)
            .filter(Subscribe.status != SubscribeStatus.COMPLETED)
            .order_by(Subscribe.id.desc())
            .all()
        )
    ...
    def get_subscribe_histories(self):
        return (
            self.db.query(Subscribe)
            .filter(Subscribe.status == SubscribeStatus.COMPLETED)
            .order_by(Subscribe.update_time.desc())
            .all()
        )
    ...
    def add_subscribe(self, param: schema.SubscribeCreate):
        ...
        if [
            exist
            for exist in exists
            if exist.num.upper() == param.num.upper()
            and exist.is_hd == param.is_hd
            and exist.is_zh == param.is_zh
            and exist.is_uncensored == param.is_uncensored
        ]:
    ...
    def do_subscribe(self):
        ...
        for subscribe in subscribes:
            ...
            def get_matched(item):
                ...
                try:
                    if subscribe.include_keyword and not re.search(
                        subscribe.include_keyword, item.name, re.IGNORECASE
                    ):
                        logger.error(f"{item.name} 不匹配包含关键字，已跳过")
                        return False

                    if subscribe.exclude_keyword and re.search(
                        subscribe.exclude_keyword, item.name, re.IGNORECASE
                    ):
    ...
    def download_video(
        self,
        video: schema.SubscribeCreate,
        link: schema.VideoDownload,
        ...
        # 确保使用正确的下载路径
        download_path = (
            link.savepath if link.savepath else setting.download.download_path
        )
        ...
        if torrent_hash:
            existing_torrent = (
                self.db.query(Torrent).filter(Torrent.hash == torrent_hash).first()
            )
    ...
    def _check_and_record_actor_subscribe_download(
        ...
        try:
            ...
            from app.db.models.actor_subscribe import (
                ActorSubscribe,
            ...
            for actor in video_detail.actors:
                ...
                actor_subscription = (
                    ...
                    .filter(ActorSubscribe.actor_name == actor.name)
                    .first()
                )

                if actor_subscription:
                    logger.info(
                        f"找到演员订阅: {actor.name} (ID: {actor_subscription.id})"
                    )
                    # 检查是否已经记录过这个下载
                    existing_download = (
                        self.db.query(ActorSubscribeDownload)
                        .filter(
                            ActorSubscribeDownload.actor_subscribe_id
                            == actor_subscription.id,
                    ...
                    if existing_download:
                        ...
                        try:
                            ...
                            continue
                        logger.info(
                            f"[智能下载] 成功记录演员订阅下载: {actor.name} - {video.num}"
                        )
                else:
