...
class ActorSubscribeService(BaseService):
        ...
        try:
            ...
            for row in result:
                ...
                subscription_dict = {
                    ...
                    "min_rating": float(getattr(row, "min_rating", 0.0))
                    if getattr(row, "min_rating", None) is not None
                    else 0.0,
                    "min_comments": int(getattr(row, "min_comments", 0))
                    if getattr(row, "min_comments", None) is not None
                    else 0,
                ...
                # 使用缓存的订阅作品总数
                subscription_dict["subscribed_works_count"] = (
                    getattr(row, "subscribed_works_count", 0) or 0
                )
            ...
            # 如果查询失败，回退到原来的简单查询
            return (
                self.db.query(ActorSubscribe).order_by(ActorSubscribe.id.desc()).all()
            )

    def get_actor_subscription_by_name(self, actor_name: str):
        """通过演员名称查找订阅"""
        return (
            self.db.query(ActorSubscribe)
            .filter(ActorSubscribe.actor_name == actor_name)
            .first()
        )

    @transaction
    def add_actor_subscription(
        self, param: schema.actor_subscribe.ActorSubscribeCreate
    ):
        # 调试日志：打印接收到的参数
        logger.info(f"[DEBUG] 接收到订阅参数: actor_name={param.actor_name}")
        logger.info(
            f"[DEBUG] min_rating={param.min_rating}, min_comments={param.min_comments}"
        )
        logger.info(
            f"[DEBUG] from_date={param.from_date}, is_hd={param.is_hd}, is_zh={param.is_zh}, is_uncensored={param.is_uncensored}"
        )
    ...
    def update_actor_subscription(
        self, param: schema.actor_subscribe.ActorSubscribeUpdate
    ):
    ...
    def update_actor_subscription_status(
        self, param: schema.actor_subscribe.ActorSubscribeStatusUpdate
    ):
        """更新演员订阅状态（暂停/恢复）"""
        exist = ActorSubscribe.get(self.db, param.id)
        if not exist:
    ...
    def delete_actor_subscription(
        self, subscription_id: int, delete_downloads: bool = False
    ):
        """删除演员订阅
        ...
        if delete_downloads and downloads:
            logger.info(
                f"开始删除演员 {exist.actor_name} 的 {len(downloads)} 个下载资源"
            )
        ...
        logger.info(
            f"已删除演员订阅: {exist.actor_name}, 删除下载资源: {delete_downloads}"
        )
        ...
        try:
            ...
            if torrent_hash:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"删除种子失败: {torrent_hash}, 状态码: {response.status_code}"
                    )
            else:
    ...
    def delete_subscription_download(
        self, download_id: int, delete_files: bool = False
    ):
        """删除单个下载记录
    ...
    def do_actor_subscribe(self):
        ...
        if len(active_subscriptions) < len(subscriptions):
            logger.info(
                f"跳过{len(subscriptions) - len(active_subscriptions)}个已暂停的订阅"
            )
        ...
        for subscription in active_subscriptions:
            try:
                ...
                logger.info(
                    f"订阅条件 - 最低评分: {subscription.get('min_rating', 0.0)}, 最低评论数: {subscription.get('min_comments', 0)}"
                )

                # 获取演员的作品列表
                actor_videos = spider.get_web_actor_videos(
                    subscription["actor_name"], "javdb"
                )
                if not actor_videos:
                    logger.error(
                        f"未获取到演员 {subscription['actor_name']} 的作品列表"
                    )
                    continue

                logger.info(
                    f"获取到演员 {subscription['actor_name']} 的 {len(actor_videos)} 个作品"
                )

                # 获取该演员已下载的作品列表
                downloaded_videos = self.get_actor_subscription_downloads(
                    subscription["id"]
                )
                ...
                for video in actor_videos:
                    total_filtered += 1
                    logger.info(
                        f"检查作品 {total_filtered}/{len(actor_videos)}: {video.get('num')} - {video.get('title', '')[:50]}"
                    )
                    ...
                    logger.info(
                        f"  评分: {rating}, 评论数: {comments_count}, 发布日期: {publish_date}"
                    )
                    ...
                    if video.get("publish_date"):
                        try:
                            ...
                            if video_date and from_date and video_date < from_date:
                                logger.info(
                                    f"  跳过: 发布日期 {video_date} 早于订阅起始日期 {from_date}"
                                )
                                continue
                        except Exception as e:
                    ...
                    if subscription.get("min_rating", 0.0) > 0.0:
                        video_rating = DataConverter.normalize_rating(
                            video.get("rating")
                        )
                        if video_rating < subscription["min_rating"]:
                            logger.info(
                                f"  跳过: 评分 {video_rating} 低于要求的 {subscription['min_rating']}"
                            )
                            continue
                    ...
                    if subscription.get("min_comments", 0) > 0:
                        ...
                        if video_comments < subscription["min_comments"]:
                            logger.info(
                                f"  跳过: 评论数 {video_comments} 低于要求的 {subscription['min_comments']}"
                            )
                            continue
                ...
                logger.info(
                    f"演员 {subscription['actor_name']} 有 {len(new_videos)} 个新作品"
                )
                ...
                for video in new_videos:
                    try:
                        ...
                        logger.error(
                            f"处理视频 {video.get('num', 'unknown')} 失败: {e}"
                        )
                        traceback.print_exc()
    ...
    def process_new_video(self, subscription: dict, video_info: dict):
        ...
        for resource in video_detail.downloads:
            if (
                (not subscription["is_hd"] or resource.is_hd)
                and (not subscription["is_zh"] or resource.is_zh)
                and (not subscription["is_uncensored"] or resource.is_uncensored)
            ):
    ...
    def select_largest_resource(self, resources):
        ...
        sorted_resources = sorted(
            resources, key=lambda x: parse_size(x.size), reverse=True
        )
    ...
    def download_actor_video(self, subscription, video_detail, resource):
        ...
        try:
            ...
            logger.info(
                f"演员订阅下载成功: {video_detail.num} - {download_result['message']}"
            )
            logger.info(
                f"过滤结果: 保留 {download_result['filtered_files']}/{download_result['total_files']} 个文件"
            )
    ...
    def update_works_count_for_subscription(self, subscription_id: int) -> bool:
        ...
        try:
            ...
            logger.info(
                f"演员 {subscription.actor_name} 的作品数量已更新: {len(qualified_videos)}"
            )
            return True
