...
from app.db.models.auto_download import (
    AutoDownloadRule,
...
class AutoDownloadService:
    ...
    @staticmethod
    def _split_csv_values(value: Optional[str]) -> Optional[List[str]]:
        if not value:
            return None
    ...
    @staticmethod
    def _get_auto_download_setting():
        return Setting().auto_download
    ...
    def _get_daily_subscription_count(self) -> int:
        today_start = self._today_start()
        return (
            self.db.query(AutoDownloadSubscription)
            .filter(AutoDownloadSubscription.created_at >= today_start)
            .count()
        )

    def _get_daily_download_attempt_count(self) -> int:
        today_start = self._today_start()
        return (
            self.db.query(AutoDownloadSubscription)
            .filter(AutoDownloadSubscription.download_time.isnot(None))
            .filter(AutoDownloadSubscription.download_time >= today_start)
            .count()
        )
    ...
    def _check_duplicate_by_resource_hash(
        self, resource_hash: str, rule_id: Optional[int] = None
    ) -> bool:
        """
        ...
        query = self.db.query(AutoDownloadSubscription).filter(
            and_(
                ...
                AutoDownloadSubscription.status
                != DownloadStatus.FAILED,  # 允许重新处理失败的订阅
            )
        )
    ...
    @staticmethod
    def _resolve_time_range_days(rule: AutoDownloadRule, force: bool = False) -> int:
        if force:
            return 365

        value = int(getattr(rule, "time_range_value", 1) or 1)
        raw_type = getattr(rule, "time_range_type", TimeRangeType.WEEK)
    ...
    @staticmethod
    def _resolve_min_rating_for_cache(rule: AutoDownloadRule) -> Optional[float]:
        if not rule.min_rating:
            return None
        value = float(rule.min_rating)
        if value > 5:
            return round(value / 2, 2)
        return value

    def execute_rules(
        self, rule_ids: Optional[List[int]] = None, force: bool = False
    ) -> Dict[str, Any]:
        """执行自动下载规则"""
        setting = self._get_auto_download_setting()
        max_daily_downloads = max(
            0, int(getattr(setting, "max_daily_downloads", 10) or 0)
        )
        remaining_quota: Optional[int] = None
        if max_daily_downloads > 0:
            today_subscriptions = self._get_daily_subscription_count()
            remaining_quota = max_daily_downloads - today_subscriptions
            if remaining_quota <= 0:
                logger.warning(
                    f"已达到每日新增订阅上限({max_daily_downloads})，跳过本次规则执行"
                )
                return {
                    "message": f"已达到每日上限({max_daily_downloads})",
                    "processed_count": 0,
                    "new_subscriptions": 0,
                }

        # 获取要执行的规则
        query = self.db.query(AutoDownloadRule).filter(
        ...
        for rule in rules:
            try:
                ...
                logger.info(
                    f"规则条件: 评分>={rule.min_rating or '无限制'}, 评论>={rule.min_comments or '无限制'}, 高清={rule.is_hd}, 中文字幕={rule.is_zh}, 无码={rule.is_uncensored}"
                )
                # 调试日志: 检查参数类型(用于排查 Decimal vs Float 问题)
                logger.debug(
                    f"规则参数类型: min_rating={type(rule.min_rating).__name__}, min_comments={type(rule.min_comments).__name__}"
                )

                # 计算时间范围
                time_range_days = self._resolve_time_range_days(rule, force=force)

                # 从缓存数据库查询视频（无需实时爬取，使用预抓取的数据）
                videos = cache_service.query_videos(
                    min_rating=self._resolve_min_rating_for_cache(rule),
                    min_comments=rule.min_comments,
                    is_hd=rule.is_hd,
                    is_zh=rule.is_zh,
                    is_uncensored=True if rule.is_uncensored else None,
                    days=time_range_days,
                    limit=200,  # 限制返回数量以提高性能
                    required_actor_id=getattr(rule, "actor_id", None),
                    required_tags=self._split_csv_values(getattr(rule, "tags", "")),
                    exclude_tags=self._split_csv_values(
                        getattr(rule, "exclude_tags", "")
                    ),
                )
                ...
                for i, video in enumerate(videos[:3]):
                    logger.info(
                        f"视频 {i + 1}: {video.get('num')} - 评分: {video.get('rating')} - 评论: {video.get('comments')} - 评论数: {video.get('comments_count')}"
                    )

                # 额外筛选（比如排除已订阅的）
                filtered_videos = self._filter_videos(rule, videos)
                logger.info(
                    f"规则 [{rule.name}] 最终筛选得到 {len(filtered_videos)} 个待处理视频"
                )
                ...
                for video in filtered_videos:
                    try:
                        if remaining_quota is not None and remaining_quota <= 0:
                            logger.info(
                                f"规则 [{rule.name}] 命中每日新增订阅上限，停止继续创建"
                            )
                            break
                ...
                logger.info(
                    f"规则 [{rule.name}] 执行完成，新增 {rule_new_subscriptions} 个订阅"
                )
                processed_count += 1
        ...
        logger.info(
            f"规则执行完成，共处理 {processed_count} 个规则，新增 {new_subscriptions} 个订阅"
        )
    ...
    def _get_candidate_videos(
        self, rule: AutoDownloadRule, force: bool = False
    ) -> List[Dict[str, Any]]:
        """获取候选视频"""
        try:
            ...
            videos = cache_service.query_videos(
                ...
                required_actor_id=getattr(rule, "actor_id", None),
                required_tags=self._split_csv_values(getattr(rule, "tags", "")),
                exclude_tags=self._split_csv_values(getattr(rule, "exclude_tags", "")),
            )
    ...
    def _filter_videos(
        self, rule: AutoDownloadRule, videos: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """筛选符合条件的视频"""
        filtered_videos = []
        ...
        for video in videos:
            ...
            if video.get("detail_missing", False):
                logger.warning(
                    f"视频 {video.get('num')} 的详细信息获取失败，暂时保留并稍后重试获取详情"
                )
            ...
            if self._check_duplicate_by_resource_hash(resource_hash, rule.id):
                logger.debug(
                    f"跳过重复影片（跨规则）: {video.get('num')} - {video.get('title')}"
                )
                continue
            ...
            if self._check_duplicate_by_num(rule.id, video.get("num")):
                logger.debug(
                    f"跳过重复影片（同规则）: {video.get('num')} - {video.get('title')}"
                )
                continue
    ...
    def _create_subscription(
        self, rule: AutoDownloadRule, video: Dict[str, Any]
    ) -> bool:
        """创建订阅记录"""
        try:
            num = video.get("num")
            ...
            existing = (
                ...
                .filter(
                    and_(
                        ...
                        AutoDownloadSubscription.num == num,
                        AutoDownloadSubscription.status
                        != DownloadStatus.FAILED,  # 允许重新创建失败的订阅
                    )
                )
                .first()
            )
    ...
    def run_cycle(
        ...
        result = self.execute_rules(rule_ids=rule_ids, force=force)
    ...
    def update_subscription_status(self):
        ...
        try:
            # 获取所有下载中的订阅
            downloading_subscriptions = (
                self.db.query(AutoDownloadSubscription)
                .filter(AutoDownloadSubscription.status == DownloadStatus.DOWNLOADING)
                .all()
            )
            ...
            for subscription in downloading_subscriptions:
                try:
                    ...
                    torrent_record = (
                        ...
                        .filter(Torrent.num == subscription.num)
                        .first()
                    )
                    ...
                    if not qb_torrent:
                        error_msg = (
                            f"订阅 {subscription.num} 在qBittorrent中没有找到对应种子"
                        )
                        logger.warning(error_msg)
                        subscription.status = DownloadStatus.FAILED
                        subscription.error_message = error_msg
                    ...
                    if "整理成功" in torrent_tags:
                        ...
                        self.db.commit()
                        logger.info(
                            f"订阅 {subscription.num} 下载并整理完成，状态已更新"
                        )
                        ...
                        self.db.commit()
                        logger.info(
                            f"订阅 {subscription.num} 种子状态异常({torrent_state})，状态已更新为失败"
                        )
    ...
    def job_auto_download():
        ...
        try:
            ...
            with SessionFactory() as db:
                service = AutoDownloadService(db=db)
    ...
    def _process_pending_subscriptions(self):
        ...
        query_limit = 200
        if max_daily_downloads > 0:
            today_download_attempts = self._get_daily_download_attempt_count()
            remaining_download_quota = max_daily_downloads - today_download_attempts
            if remaining_download_quota <= 0:
                logger.info(
                    f"已达到每日下载尝试上限({max_daily_downloads})，跳过待下载处理"
                )
                return
            query_limit = min(query_limit, remaining_download_quota)

        # 获取待下载的订阅
        pending_subscriptions = (
            self.db.query(AutoDownloadSubscription)
            .filter(AutoDownloadSubscription.status == DownloadStatus.PENDING)
            .limit(query_limit)
            .all()
        )  # 限制数量避免过载
    ...
    def _find_suitable_download(
        self, rule: AutoDownloadRule, downloads: List[Any]
    ) -> Optional[Any]:
        """找到合适的下载链接"""
        ...
        def matches_criteria(download):
            ...
            if (
                rule
                and rule.is_uncensored
                and not getattr(download, "is_uncensored", False)
            ):
                return False
            return True
    ...
    def _process_single_subscription(self, subscription):
        ...
        try:
            ...
            # 筛选合适的下载链接
            rule = (
                self.db.query(AutoDownloadRule)
                .filter(AutoDownloadRule.id == subscription.rule_id)
                .first()
            )
            suitable_download = self._find_suitable_download(
                rule, video_detail.downloads
            )
            ...
            # 执行下载
            try:
                auto_download_setting = self._get_auto_download_setting()
                subscribe_service.download_video(
                    subscribe_data,
                    suitable_download,
                    send_notification=bool(
                        getattr(auto_download_setting, "notification_enabled", True)
                    ),
                )
            except Exception as download_error:
                error_msg = (
                    f"视频 {subscription.num} 下载服务调用失败: {str(download_error)}"
                )
            ...
            # 更新订阅状态和下载链接
            subscription.download_url = getattr(
                suitable_download, "magnet", None
            ) or getattr(suitable_download, "url", None)
            self.db.commit()
        ...
        try:
            ...
            if query.is_enabled is not None:
                base_query = base_query.filter(
                    AutoDownloadRule.is_enabled == query.is_enabled
                )

            if query.name:
                base_query = base_query.filter(
                    AutoDownloadRule.name.like(f"%{query.name}%")
                )
            ...
            # 应用分页
            rules = (
                base_query.offset((query.page - 1) * query.page_size)
            ...
            for rule in rules:
                ...
                subscription_count = (
                    self.db.query(AutoDownloadSubscription)
                    .filter(AutoDownloadSubscription.rule_id == rule.id)
                    .count()
                )
    ...
    def get_subscriptions(self, query):
        ...
        try:
            ...
            base_query = self.db.query(
                AutoDownloadSubscription, AutoDownloadRule.name.label("rule_name")
            ).outerjoin(
                AutoDownloadRule,
            ...
            if query.rule_id:
                base_query = base_query.filter(
                    AutoDownloadSubscription.rule_id == query.rule_id
                )

            if query.status:
                base_query = base_query.filter(
                    AutoDownloadSubscription.status == query.status
                )

            if query.num:
                base_query = base_query.filter(
                    AutoDownloadSubscription.num.like(f"%{query.num}%")
                )

            if query.start_date:
                base_query = base_query.filter(
                    func.date(AutoDownloadSubscription.created_at) >= query.start_date
                )

            if query.end_date:
                base_query = base_query.filter(
                    func.date(AutoDownloadSubscription.created_at) <= query.end_date
                )
            ...
            # 应用分页
            subscriptions = (
                base_query.order_by(AutoDownloadSubscription.created_at.desc())
            ...
            for subscription_tuple in subscriptions:
                ...
                try:
                    ...
                    subscription_response = (
                        AutoDownloadSubscriptionResponse.model_validate(
                            subscription_dict
                        )
                    )
                except AttributeError:
                    # Pydantic v1
                    subscription_response = AutoDownloadSubscriptionResponse(
                        **subscription_dict
                    )
    ...
    def update_rule(self, rule_data):
        ...
        try:
            ...
            rule = (
                self.db.query(AutoDownloadRule)
                .filter(AutoDownloadRule.id == rule_data.id)
                .first()
            )
    ...
    def delete_rule(self, rule_id):
        ...
        try:
            ...
            rule = (
                self.db.query(AutoDownloadRule)
                .filter(AutoDownloadRule.id == rule_id)
                .first()
            )
    ...
    def delete_subscription(self, subscription_id):
        ...
        try:
            ...
            subscription = (
                self.db.query(AutoDownloadSubscription)
                .filter(AutoDownloadSubscription.id == subscription_id)
                .first()
            )
        ...
        try:
            ...
            if not operation.ids and operation.action == "retry":
                failed_subscriptions = (
                    self.db.query(AutoDownloadSubscription)
                    .filter(AutoDownloadSubscription.status == DownloadStatus.FAILED)
                    .all()
                )
            ...
            logger.info(
                f"开始批量操作 '{operation.action}'，涉及 {len(operation.ids)} 个记录"
            )
            ...
            for subscription_id in operation.ids:
                try:
                    ...
                    if operation.action == "delete":
                        logger.info(
                            f"删除订阅记录: {subscription.num} (ID: {subscription_id})"
                        )
                        self.db.delete(subscription)
                        ...
                        if subscription.status == DownloadStatus.FAILED:
                            ...
                            subscription.error_message = None
                            logger.info(
                                f"重试失败记录: {subscription.num} (ID: {subscription_id})"
                            )
                            ...
                            try:
                                ...
                                error_msg = (
                                    f"立即处理重试订阅失败: {str(process_error)}"
                                )
                                logger.error(error_msg)
                                subscription.status = DownloadStatus.FAILED
                                subscription.error_message = error_msg
                            ...
                            logger.warning(
                                f"订阅记录状态不是失败，无法重试: {subscription.num} (当前状态: {subscription.status})"
                            )
                    elif operation.action == "pause":
                        if subscription.status != DownloadStatus.COMPLETED:
                            subscription.status = DownloadStatus.FAILED
                            logger.info(
                                f"暂停订阅记录: {subscription.num} (ID: {subscription_id})"
                            )
                        else:
                        ...
                        if subscription.status == DownloadStatus.FAILED:
                            ...
                            subscription.error_message = None
                            logger.info(
                                f"恢复订阅记录: {subscription.num} (ID: {subscription_id})"
                            )
                        else:
                            logger.warning(
                                f"只能恢复失败状态的订阅: {subscription.num} (当前状态: {subscription.status})"
                            )
            ...
            logger.info(
                f"批量操作 '{operation.action}' 完成: 成功 {success_count} 个，失败 {failed_count} 个"
            )
    ...
    def preview_rule(self, rule_id: int, limit: int = 10) -> Dict[str, Any]:
        rule = (
            self.db.query(AutoDownloadRule)
            .filter(AutoDownloadRule.id == rule_id)
            .first()
        ...
        videos = self._get_candidate_videos(rule)
        filtered_videos = self._filter_videos(rule, videos)
        preview_items = filtered_videos[:limit]
        return {
            "rule_id": rule_id,
            "rule_name": rule.name,
            "candidate_count": len(videos),
            "matched_count": len(filtered_videos),
            "preview_videos": preview_items,
        }
    ...
    def get_statistics(self):
        ...
        try:
            ...
            active_rules = (
                self.db.query(func.count(AutoDownloadRule.id))
                .filter(AutoDownloadRule.is_enabled == True)
                .scalar()
                or 0
            )
            ...
            # 获取订阅统计
            total_subscriptions = (
                self.db.query(func.count(AutoDownloadSubscription.id)).scalar() or 0
            )
            ...
            for status in [
                ...
                count = (
                    self.db.query(func.count(AutoDownloadSubscription.id))
                    .filter(AutoDownloadSubscription.status == status)
                    .scalar()
                    or 0
                )
            ...
            statistics = {
                ...
                "downloading_subscriptions": status_counts.get(
                    DownloadStatus.DOWNLOADING, 0
                ),
                "completed_subscriptions": status_counts.get(
