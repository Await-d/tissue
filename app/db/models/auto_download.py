...
class AutoDownloadRule(Base):
    ...
    time_range_type = Column(
        String(20), nullable=True, default="week", comment="时间范围类型"
    )
    ...
    subscriptions = relationship(
        "AutoDownloadSubscription", back_populates="rule", cascade="all, delete-orphan"
    )
...
class AutoDownloadSubscription(Base):
    ...
    rule_id = Column(
        Integer,
        ForeignKey("auto_download_rules.id", ondelete="CASCADE"),
        nullable=False,
        comment="关联规则ID",
    )
    ...
    resource_hash = Column(
        String(64), nullable=True, comment="资源唯一标识（用于重复检测）", index=True
    )
