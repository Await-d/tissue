...
import { testQBittorrentConnection } from '@/apis/setting'
...
// 表单初始值（迁移前 settings useState 的初始值）：
// 远程配置未加载或区块缺字段时用它兜底，保证首帧与旧页面一致。
...
function AutoDownloadSettings() {
  ...
  const { form, saving, submit } = useSectionSettings('auto_download')
  // 手动触发按钮的 loading（原与保存共用；迁移后保存改用 hook 的 saving）
  ...
  const handleTest = async () => {
    try {
      setTesting(true)
      const res = await testQBittorrentConnection()
      ...
      message.error('测试连接失败：' + ((error as Error).message || '未知错误'))
    } finally {
      setTesting(false)
    }
  }
  ...
  return (
    <SettingsPage
      ...
      cardBorder="solid"
      formInitialValues={DEFAULT_SETTINGS}
      footerClassName="flex justify-center gap-4 pt-6 border-t"
      beforeCard={
        /* 运行状态卡片 */
        statistics && (
