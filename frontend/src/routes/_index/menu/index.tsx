...
// MenuItemCard 的 props：colors 由父组件传入，主题切换时 memo 仍能正确重渲染
...
// MenuItem 卡片组件 - 提升到模块作用域并用 React.memo 包裹，避免每次 Menu 渲染重建组件类型导致所有卡片重新挂载
const MenuItemCard = React.memo(function MenuItemCard({ item, groupIndex, itemIndex, colors }: MenuItemCardProps) {
                    ...
                    {group.items.map((itemKey, itemIndex) => {
                        ...
                        return item ? (
                            <Col key={item.key} span={6}>
                                <MenuItemCard item={item} groupIndex={index} itemIndex={itemIndex} colors={colors} />
                            </Col>
                        ) : null;
                    })}
                </Row>
